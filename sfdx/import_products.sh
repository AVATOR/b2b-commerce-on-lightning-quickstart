...
then
    ...
    storeId=`sf project convert mdapi -q "SELECT Id FROM WebStore WHERE Name='$1' LIMIT 1" -r csv |tail -n +2`
	...
	then
		...
		sf data import tree -p json/Productless-Plan-1.json
		...
		do
	   		:
			sf data create record -s CommerceEntitlementProduct -v "PolicyId='${policyID}' ProductId='${i}'"
		done
		...
		sf data create record -s WebStoreCatalog -v "ProductCatalogId='${catalogId}' SalesStoreId='${storeId}'"
		...
		sf data create record -s WebStorePricebook -v "IsActive=true Pricebook2Id='${pricebook2Id}' WebStoreId='${storeId}'"
		...
		sf data create record -s BuyerGroupPricebook -v "Pricebook2Id='${pricebook2Id}' BuyerGroupId='${buyergroupId}'"
		...
		# Import files
		sf data import tree -p json/Plan-1.json
	fi
